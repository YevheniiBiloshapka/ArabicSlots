...
export const ttfToWoff = () => {
	return app.gulp.src(`${app.path.srcFolder}/fonts/*.ttf`, {})
		...
		.pipe(app.gulp.dest(`${app.path.build.fonts}`)) // Копируем .ttf в папку build
		.pipe(ttf2woff2())
		.pipe(app.gulp.dest(`${app.path.build.fonts}`))
			...
			if (!fs.existsSync(fontsFile)) {
				...
				for (var i = 0; i < fontsFiles.length; i++) {
					...
					if (newFileOnly !== fontFileName) {
						...
						// Добавляем .ttf первым, затем woff2 и woff
						fs.appendFile(fontsFile, `@font-face {\n\tfont-family: '${fontName}';\n\tfont-display: swap;\n\tsrc: url("../fonts/${fontFileName}.ttf") format("truetype"), url("../fonts/${fontFileName}.woff2") format("woff2"), url("../fonts/${fontFileName}.woff") format("woff");\n\tfont-weight: ${fontWeight};\n\tfont-style: normal;\n}\r\n`, cb);
